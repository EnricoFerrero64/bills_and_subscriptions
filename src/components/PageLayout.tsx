...
import { CreditCard, BarChart2, Receipt, Settings, X, RefreshCw, Link } from "lucide-react";
...
interface Tab {
  ...
  subTabs?: { label: string; path: string; icon: ReactNode }[];
}
...
const ALL_TABS: Tab[] = [
  ...
  { label: "Bills",         path: "/addons/bills-and-subscriptions/bills",   icon: <Receipt    className="h-3.5 w-3.5" />, settingKey: "billsEnabled" },
  {
    ...
    subTabs: [
      { label: "Suggestions", path: "/addons/bills-and-subscriptions/bill-linking/suggestions", icon: <Link className="h-3.5 w-3.5" /> },
      { label: "Linked", path: "/addons/bills-and-subscriptions/bill-linking/links", icon: <Link className="h-3.5 w-3.5" /> },
    ],
  },
];
...
export function PageLayout({ children, activePath }: PageLayoutProps) {
  ...
  const handleSubTabClick = (path: string) => {
    ctx.api.navigation.navigate(path);
  };
  ...
  const showSubTabs = !!(currentTab && currentTab.subTabs);
    ...
    <div className="bills-and-subscriptions-root flex flex-col min-h-screen bg-background text-foreground">
      ...
      <div className="px-4 pt-4 pb-3 flex flex-col gap-3 border-b border-border">
        ...
        >
          ...
          {tabs.map((tab) => {
            const isActive = activePath === tab.path || (tab.subTabs && tab.subTabs.some(st => st.path === activePath));
            return (
              <button
                key={tab.path}
                data-path={tab.path}
        ...
        {/* Sub-tabs (for Bill Linking) */}
        {showSubTabs && currentTab && currentTab.subTabs && (
          <div className="flex items-center gap-2 self-start">
            <div className="flex items-center rounded-lg p-0.5 bg-muted/50">
              {currentTab.subTabs.map(st => {
                const isActive = activePath === st.path;
                return (
                  <button
                    key={st.path}
                    onClick={() => handleSubTabClick(st.path)}
